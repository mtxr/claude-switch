...
def keychain_delete(service):
    subprocess.run(
        ["security", "delete-generic-password", "-s", service], capture_output=True
    )
...
def _switch_link(link, target, ensure_dir=False):
    ...
    if link.is_symlink():
        ...
        die(
            f"{link} exists and is not a symlink. Run '{PREF_CMD} save <name>' first to migrate."
        )
...
def _ensure_current_saved():
    ...
    print(
        f"⚠️  Your current ~/.claude.json and ~/.claude/ are not managed by {PREF_CMD}."
    )
    save_name = input(
        "Enter a name to save the current profile (or press Enter to skip): "
    ).strip()
    if save_name:
        cmd_save(save_name)
